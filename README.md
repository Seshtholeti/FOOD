    ...
    const fetchData = async () => {
      try {
        ...
        for (let i = 0; i < responseData.body.length; i++) {
          if (
            responseData.body[i]["DEPARTMENT"] === "Queries" ||
            responseData.body[i]["DEPARTMENT"] === "Reservation"
          ) {
            avgTemp = avgTemp + responseData.body[i]["ANS_RATE"];
          }
        }
        console.log(avgTemp);
